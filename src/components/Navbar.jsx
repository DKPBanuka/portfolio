...
import { FaBars, FaTimes, FaSun, FaMoon } from 'react-icons/fa'; // Hamburger menu icons and theme icons
    ...
    <nav className="sticky top-3 z-50">
      ...
      <div className="flex justify-center px-4">
        <div className="w-full max-w-6xl ">
          <div
            ref={containerRef}
            className="relative flex items-center justify-between rounded-3xl bg-white/80 dark:bg-gray-900/80 backdrop-blur-2xl shadow-[0_8px_30px_-18px_rgba(0,0,0,.10)] dark:shadow-[0_8px_30px_-18px_rgba(255,255,255,.10)] ring-1 ring-pink-50 dark:ring-pink-900 border border-pink-50/40 dark:border-pink-900/40 px-5 py-2 overflow-hidden"
            onMouseMove={(e) => {
              const container = containerRef.current;
          ...
          <div className="hidden md:flex items-center gap-6 flex-1 justify-center">
            {navLinks.map((link) => (
              <Link
                ...
                activeClass="bg-[#ff0055] dark:bg-[#ff0055] text-white dark:text-white"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="hidden md:flex items-center gap-2">
            <button
              ...
              onClick={toggleTheme}
              className="grid h-10 w-10 place-items-center rounded-2xl bg-gradient-to-br from-[#e6004c] to-pink-500 text-white shadow-lg transition-transform hover:scale-105"
            >
          ...
          <div className="md:hidden flex items-center gap-2">
            ...
            <button
              aria-label="Toggle theme"
              onClick={toggleTheme}
              className="grid h-10 w-10 place-items-center rounded-2xl bg-gradient-to-br from-[#e6004c] to-pink-500 text-white shadow-lg transition-transform"
            >
              {theme === 'dark' ? <FaSun size={20} /> : <FaMoon size={20} />}
            </button>
        ...
        <div className={`md:hidden fixed inset-0 z-40 transition ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
          {/* Backdrop */}
          <div
            className={`absolute inset-0 bg-black/30 dark:bg-black/60 transition-opacity ${isOpen ? 'opacity-100' : 'opacity-0'}`}
            onClick={toggleMenu}
          />
          {/* Panel */}
          <div
            className={`absolute left-0 top-0 h-full w-96 bg-white/30 dark:bg-gray-900/30 backdrop-blur-xl border-l-4 border-pink-300 dark:border-pink-600 shadow-2xl rounded-r-3xl transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
          >
            {/* Close button for mobile drawer */}
            <button
              aria-label="Close menu"
              className="absolute top-4 right-4 z-50 bg-white/80 dark:bg-gray-800/80 rounded-full p-2 shadow-lg hover:bg-pink-100 dark:hover:bg-pink-900 transition"
              onClick={toggleMenu}
            >
                ...
                {navLinks.map(link => {
                  ...
                  return (
                    <a
                      ...
                      className={`flex items-center gap-4 px-4 py-3 rounded-2xl shadow-lg backdrop-blur-lg transition-all ${isActive ? 'bg-pink-200/60 dark:bg-pink-800/60 scale-105' : 'bg-white/40 dark:bg-gray-800/40 hover:bg-pink-100 dark:hover:bg-pink-900'}`}
                      style={{
                    ...
                    >
                      <span className="bg-white/60 dark:bg-gray-700/60 rounded-xl p-3 shadow flex items-center justify-center">{icons[link.to]}</span>
              ...
              <div className="flex flex-col items-center gap-2">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="h-16 w-16 rounded-full overflow-hidden shadow-lg">
                    <img src={PBLogo} alt="Profile" className="h-full w-full object-cover rounded-full border-4 border-pink-200 dark:border-pink-800" />
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-gray-700 dark:text-gray-200">Call Now</div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-gray-700 dark:text-gray-200">Mail Us</div>
